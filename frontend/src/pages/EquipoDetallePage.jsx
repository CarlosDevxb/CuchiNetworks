...
const EquipoDetallePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const ubActual = resUbicaciones.data.find(
          (u) => u.id === data.ubicacion_id
        );
  ...
  const handleSave = async () => {
    try {
      ...
      setEquipo({
        ...
        ubicacion_nombre:
          ubicacionSeleccionada?.nombre || equipo.ubicacion_nombre, // Actualizar nombre visual
  ...
  const renderSpecs = () => {
    ...
    const d =
      typeof equipo.detalles === "string"
        ? JSON.parse(equipo.detalles)
        : equipo.detalles;
    ...
    if (equipo.tipo === "computadora") {
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 animate-fade-in">
          ...
          <div className="bg-purple-50 p-4 rounded-2xl border border-purple-100">
            <h4 className="font-bold text-purple-800 mb-2 flex items-center gap-2">
              <Monitor size={18} /> Periféricos
            </h4>
            <div className="flex gap-3 text-sm">
              <span
                className={`flex items-center gap-1 ${
                  d.perifericos?.mouse ? "text-green-600" : "text-gray-400"
                }`}
              >
                {d.perifericos?.mouse ? (
                  <CheckCircle size={14} />
              ...
              <span
                className={`flex items-center gap-1 ${
                  d.perifericos?.teclado ? "text-green-600" : "text-gray-400"
                }`}
              >
                {d.perifericos?.teclado ? (
                  <CheckCircle size={14} />
                ) : (
                  <XCircle size={14} />
                )}{" "}
      ...
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 animate-fade-in">
          <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
            <h4 className="font-bold text-cuchi-primary mb-2 flex items-center gap-2">
              <RouterIcon size={18} /> Interfaces
            </h4>
            <div className="flex justify-between text-sm text-gray-600">
              <div className="text-center">
                <span className="block font-bold text-lg">
                  {d.interfaces?.fastEthernet || 0}
                </span>
                Fast
              </div>
              <div className="text-center">
                <span className="block font-bold text-lg">
                  {d.interfaces?.serial || 0}
                </span>
                Serial
              </div>
              <div className="text-center">
                <span className="block font-bold text-lg">
                  {d.interfaces?.gigabit || 0}
                </span>
                Giga
              </div>
            </div>
          ...
          <div className="flex gap-4">
            <label>
              <input
                type="checkbox"
                name="tiene_mouse"
                checked={detallesData.tiene_mouse}
                onChange={handleDetalleChange}
              />{" "}
              Mouse
            </label>
            <label>
              <input
                type="checkbox"
                name="tiene_teclado"
                checked={detallesData.tiene_teclado}
                onChange={handleDetalleChange}
              />{" "}
              Teclado
            </label>
        ...
        <div className="bg-blue-50 p-4 rounded-2xl mt-4 border border-blue-200">
          <div className="grid grid-cols-3 gap-2 mb-4">
            <label className="text-xs block">
              ...
              <input
                type="number"
                name="interfaces_fast"
                value={detallesData.interfaces_fast}
                onChange={handleDetalleChange}
            ...
            <label className="text-xs block">
              ...
              <input
                type="number"
                name="interfaces_serial"
                value={detallesData.interfaces_serial}
                onChange={handleDetalleChange}
                className="w-full p-1 border rounded"
              />
            </label>
            <label className="text-xs block">
              ...
              <input
                type="number"
                name="interfaces_giga"
                value={detallesData.interfaces_giga}
                onChange={handleDetalleChange}
                className="w-full p-1 border rounded"
              />
            </label>
          ...
          <div className="flex gap-4">
            <label>
              <input
                type="checkbox"
                name="tiene_cable_consola"
                checked={detallesData.tiene_cable_consola}
                onChange={handleDetalleChange}
              />{" "}
              Consola
            </label>
            <label>
              <input
                type="checkbox"
                name="tiene_cable_corriente"
                checked={detallesData.tiene_cable_corriente}
                onChange={handleDetalleChange}
              />{" "}
              Corriente
            </label>
  ...
  const renderSelectorPosicion = () => {
    if (!ubicacionSeleccionada)
      return <div className="p-2 text-xs text-gray-400">Selecciona Zona</div>;

    if (ubicacionSeleccionada.tipo_zona === "isla") {
      return (
        <select
          name="posicion_fisica"
          value={formData.posicion_fisica}
    ...
    return (
      <input
        ...
        placeholder="Ej. Estación 5"
      />
    );
  };

  if (loading)
    return <div className="flex justify-center mt-20">Cargando...</div>;
  ...
  return (
    <div className="fade-in max-w-6xl mx-auto pb-10 font-sans">
      <button
        ...
        className="flex items-center text-gray-500 mb-6 hover:text-cuchi-primary"
      >
        ...
        <div className="flex-1 w-full">
              ...
              {isEditing ? (
                <div className="space-y-3">
                  <input
                    ...
                    onChange={handleInputChange}
                    className="text-3xl font-bold w-full border-b-2 border-cuchi-primary focus:outline-none"
                  />
                  <input
                    type="text"
                    name="modelo"
                    ...
                    onChange={handleInputChange}
                    className="text-xl text-gray-500 w-full border-b focus:outline-none"
                    placeholder="Modelo"
                  />
                </div>
              ) : (
                <>
                  <div className="flex gap-2 mb-3">
                    <span className="bg-blue-50 text-cuchi-primary px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                      {equipo.tipo}
                    </span>
                    <span
                  ...
                  <h1 className="text-4xl font-bold text-cuchi-text mb-2">
            ...
            <div className="flex gap-2">
              {isEditing ? (
                <>
                  <button
                    onClick={handleSave}
                    className="p-3 rounded-xl bg-cuchi-primary text-white shadow-lg hover:bg-blue-700 transition-all"
                  >
                    <Save size={20} />
                  </button>
                  <button
                ...
                <button
                  ...
                  className="p-3 rounded-xl bg-white border border-gray-200 text-gray-500 hover:text-cuchi-primary hover:border-cuchi-primary transition-colors shadow-sm"
                >
                  <Edit size={20} />
                </button>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-6 border-t border-b border-gray-50 my-6">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-blue-50 text-cuchi-primary rounded-xl">
                <MapPin size={24} />
              </div>
              <div className="w-full">
                ...
                {isEditing ? (
                  ...
                  <div>
                    <p className="text-cuchi-text font-bold text-lg">
                      {equipo.ubicacion_nombre || "Sin asignar"}
                    </p>
                    {equipo.posicion_fisica && (
                      <p className="text-cuchi-primary text-sm font-bold mt-1">
                        {equipo.posicion_fisica}
                      </p>
                    )}
                  </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-cuchi-text font-mono font-medium text-lg">
